...
void getCatmullRomPoint(float t, float *p0, float *p1, float *p2, float *p3, float *pos, float *deriv) {
	...
	multMatrixVector(*a, tVector1, pos);
	
	// compute deriv = T' * A
	float tVector2[4] = {3 * powf(t, 2), 2 * t, 1, 0};
	multMatrixVector(*a, tVector2, deriv);
...
void renderScene(void) {
	...
	static float t = 0;
	float pos[4], deriv[4], direction[4], rotMatrix[16];
	...
	getGlobalCatmullRomPoint((i++%1000)/1000.0, pos, deriv);

	normalize(deriv);
    cross(deriv, up, direction);
    normalize(direction);
	cross(direction, deriv, up);
	normalize(up);

    buildRotMatrix(deriv, up, direction, rotMatrix);
